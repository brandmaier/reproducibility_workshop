...
Always avoid local paths. Probably (*not*) a good idea to add this to your user-wide `.Rprofile` (`usethis::edit_r_profile()`):
...
- Multiple authors can commit to their local repositories and synchronize with a central repository (automated merge of non-conflicting changes)
...
sorted to ` 01aAbB` in German, `01AaBb` in Norwegian, 01ABa in `Korean`
...
- We need to better recognize the efforts of maintaining scientific software (such as repro and many others) and curating digital collections and community services `r Citep(bib,"brandmaier2024assessing")`
...
If you want to engage:
  ...
  - Follow [@brandmaier](https://twitter.com/brandmaier) on twitter or .strong[@brandmaier.bsky.social]
